...
motif_perturbs.path<-'tsv/mapped_motifs/all_instances_curated_0based_w_perturb.tsv.gz'
paired_perturbs.path<-'tsv/genomic/all_pairs_curated_0based_w_perturb.tsv.gz'

alternate_perturbs.path<-'tsv/genomic/all_pairs_curated_0based_w_alternate_perturb.tsv.gz'
...
```{r}
perturbs_processed.df<-readr::read_tsv('tsv/mapped_motifs/all_pairs_curated_0based_with_cooperativity.tsv.gz')
```
...
First, select the top, middle, and bottom context scores of pioneers (independent of motif identity). We are using context scores because they show that the context-considered in vivo features of whether a motif will be reactive and cooperative relative to other features. 
...
perturb_candidates.df<-perturbs.df %>%
  dplyr::filter(model_name == 'atac_wt', task == 'atac', motif_pair_distance<=200, motif_pair_distance>=20) %>%
  dplyr::mutate(atac_wt_perturb_A = log(hAB_sum) - log(hA_sum),
                atac_wt_perturb_B = log(hAB_sum) - log(hB_sum)) %>%
  dplyr::filter(atac_wt_perturb_A>0, atac_wt_perturb_B>0, 
                motif.x %in% c('Oct4-Sox2','Sox2', 'Klf4'),
                motif.y %in% c('Oct4-Sox2','Sox2', 'Klf4')) %>% #c('Sox2')) %>% #
  dplyr::mutate(atac_wt_perturb_A_q = ecdf(atac_wt_perturb_A)(atac_wt_perturb_A),
                atac_wt_perturb_B_q = ecdf(atac_wt_perturb_A)(atac_wt_perturb_B))%>% 
  dplyr::filter(motif_pair_ordered==motif_pair_blind) %>% #remove redundant motifs
  dplyr::mutate(atac_wt_perturb_A_group = cut(atac_wt_perturb_A_q, 
                                              breaks = c(-Inf, outlier, lower_range, .5-(mid_range/2), 
                                                         .5+(mid_range/2), (1-upper_range), (1-outlier), Inf),
                                              labels = c('remove', 'q_low', 'remove', 'q_mid', 'remove', 'q_high', 'remove'), 
                                              include.lowest = T),
                atac_wt_perturb_B_group = cut(atac_wt_perturb_B_q, 
                                              breaks = c(-Inf, outlier, lower_range, .5-(mid_range/2), 
                                                         .5+(mid_range/2), (1-upper_range), (1-outlier), Inf),
                                              labels = c('remove', 'q_low', 'remove', 'q_mid', 'remove', 'q_high', 'remove'), 
                                              include.lowest = T)) %>%
  dplyr::filter(atac_wt_perturb_A_group!='remove', atac_wt_perturb_B_group!='remove') %>%
  dplyr::mutate(paired_perturb_class = paste0(`atac_wt_perturb_A_group`, ' vs ', `atac_wt_perturb_B_group`))

ggplot(perturb_candidates.df, aes(paired_perturb_class, fill = motif_pair_blind))+
...
```{r}
merged_class_index.df<-data.frame(paired_perturb_class = c("q_low vs q_low", "q_mid vs q_low", "q_mid vs q_mid", "q_low vs q_mid", "q_high vs q_mid", "q_high vs q_low", "q_mid vs q_high", "q_high vs q_high", "q_low vs q_high")) %>%
  dplyr::mutate(paired_perturb_class_merged = c("q_low vs q_low", "q_mid vs q_low", "q_mid vs q_mid", "q_mid vs q_low", "q_high vs q_mid", "q_high vs q_low", "q_high vs q_mid", "q_high vs q_high", "q_high vs q_low") %>%
...
perturb_candidates_tidy.df<-perturb_candidates.df %>% 
  ...
  tidyr::separate(paired_perturb_class_merged, into = c('left_class','right_class'), sep = ' vs ', remove = F) %>%
  dplyr::mutate(A = ifelse(paired_perturb_class_merged==paired_perturb_class, A_unmerged, B_unmerged),
                B = ifelse(paired_perturb_class_merged==paired_perturb_class, B_unmerged, A_unmerged),
                left_motif = ifelse(paired_perturb_class_merged==paired_perturb_class, `motif.x`, `motif.y`),
                right_motif = ifelse(paired_perturb_class_merged==paired_perturb_class, `motif.y`, `motif.x`)) %>%
...
perturb_candidates_summary.df<-perturb_candidates_tidy.df %>%
  dplyr::group_by(state, paired_perturb_class_merged) %>% #, motif_pair_blind) %>%
  dplyr::summarize(med_effects = median(effects)) %>% 
  dplyr::mutate(state = factor(state, levels = c('AB', 'A','B', 'null')))

uneven_levels.plot<-ggplot(perturb_candidates_summary.df, aes(x = paired_perturb_class_merged))+
...
left_uneven_motifs.plot<-perturb_candidates_tidy.df %>%
  dplyr::group_by(paired_perturb_class_merged, left_motif, right_motif) %>%
  dplyr::summarize(freq = dplyr::n()) %>%
  ggplot(., aes(x = paired_perturb_class_merged))+
...
perturbs_classed.df<-perturbs.df %>% dplyr::filter(task=='atac', motif_pair_distance >=30)  %>%
  dplyr::mutate(motif_pair_bin = ifelse(motif_pair_distance>=distance_feature, 'long', motif_pair_ordered)) %>%
  dplyr::mutate(motif_pair_bin = factor(motif_pair_bin, c('long', motif_pair_ordered_order))) %>%
...
wilcox_across_pair.df<-padj.df %>% arrange(padj_to_long) %>%
  ...
  dplyr::mutate(motif_pair_bin = factor(motif_pair_bin, motif_pair_ordered_order)) %>%
  dplyr::arrange(desc(motif_pair_bin))
wilcox_across_pair.df
```

Everything is significant, but this makes sense as we know this is distance dependent and that there are so many examples of motif pairs that even a slight increase is statistically significant. We are relatively unsurprised about this.
...
wilcox_paired_by_bin.df<-mclapply(perturbs.df$motif_pair_ordered %>% unique, function(x){
  
  p.df<-perturbs.df %>% dplyr::filter(motif_pair_ordered==x, task=='atac', motif_pair_distance >=30)  %>%
    dplyr::mutate(distance_bin = plyr::round_any(motif_pair_distance, 10, ceiling)) %>%
    dplyr::mutate(distance_bin_all = ifelse(distance_bin>=distance_feature, 'long', distance_bin)) %>%
    
    #Because we are using `pairwise.wilcox.test` it is really important that we order this data frame in this EXACT way
    #This allows us to apply the greater alternative hypothesis
    dplyr::arrange(desc(distance_bin)) %>%
    dplyr::mutate(distance_bin_all = factor(distance_bin_all, unique(distance_bin_all)))
  p.df$distance_bin_all %>% table
  
  paired.wilcox.test<-pairwise.wilcox.test(x = log(p.df$add_coop), g = p.df$distance_bin_all, p.adjust.method = "bonf", alternative = 'greater')
  padj.df<-data.frame(padj_to_long = paired.wilcox.test$p.value[,'long'])
  padj.df$distance_bin_all<-rownames(padj.df)
  padj.df$motif_pair_ordered<-x
  rownames(padj.df)<-NULL
  
  return(padj.df)
  
}, mc.cores = 8) %>% rbindlist() %>% arrange(padj_to_long) %>%
  dplyr::mutate(sig = cut(padj_to_long, breaks = c(0, 1e-7, 1), labels = c('*', ''),
  # dplyr::mutate(sig = cut(padj_to_long, breaks = c(0, 0.001, 0.01, 0.05, 1), labels = c('***','**', '*', ''), 
                          include.lowest = T, right = T)) %>%
  dplyr::select(-padj_to_long) %>%
  dplyr::mutate(motif_pair_ordered = factor(motif_pair_ordered, motif_pair_ordered_order),
                distance_bin_all = as.integer(distance_bin_all)) %>%
  dplyr::arrange(distance_bin_all, desc(motif_pair_ordered)) %>%
  tidyr::pivot_wider(names_from = motif_pair_ordered, values_from = sig)

print(wilcox_paired_by_bin.df %>% as.data.frame())
# View(wilcox_paired_by_bin.df %>% as.data.frame())
```

Here, we see ranges of about 240bp providing significant cooperativity.
...
```{r}
sig_marked.df<-wilcox_paired_by_bin.df %>% dplyr::rename(distance_bin = distance_bin_all) %>%
  tidyr::pivot_longer(cols = -distance_bin, names_to = 'motif_pair_ordered', values_to = 'sig')

summarize_all_distance.plot<-perturbs.df   %>%
  dplyr::mutate(distance_bin = plyr::round_any(motif_pair_distance, 10, ceiling)) %>%
  dplyr::group_by(task, motif_pair_ordered, distance_bin) %>%
  ...
  ggplot(.)+
  # geom_tile()+
  ggrastr::geom_tile_rast(aes(x = distance_bin, y = motif_pair_ordered, fill = log(coop)))+
  geom_text(data = sig_marked.df, aes(x = distance_bin, y = motif_pair_ordered, label = sig), color = 'black')+
...
Selected motif pairs with distance.
...
#### Alternate models and cooperativity

Read in paired perturbations and process for all motifs for alternate models.

```{r}
alternate_perturbs.df<-readr::read_tsv(alternate_perturbs.path) %>%
    dplyr::mutate(motif_pair_distance = abs(pattern_center.y - pattern_center.x)) %>%
    dplyr::rowwise() %>%
    dplyr::mutate(motif_pair_ordered = paste(sort(c(motif.x, motif.y)), collapse = " vs "),
                  motif_pair_blind = paste0(motif.x, ' vs ', motif.y)) %>% 
...
Per model, process the paired perturbations. 

```{r}
alternate_model_summary.df<-mclapply(c('atac_wt_across_genome', 'atac_wt_across_acc', 'atac_wt_across_split'), function(x){
  message(x)
  task<-'atac'
  p_summary.df<- alternate_perturbs.df %>%
    dplyr::select(region_id, pair_name, pair_id, motif_pair_ordered, motif_pair_blind, motif_pair_distance, seq.x, seq.y,
                  motif.x, motif.y, seq_match_quantile.x, seq_match_quantile.y,
                  hAB_sum = !!sym(paste0(x, '/', task, '/hAB/pred_sum')),
                  hB_sum = !!sym(paste0(x, '/', task, '/hB/pred_sum')),
                  hA_sum = !!sym(paste0(x, '/', task, '/hA/pred_sum')),
                  null_sum = !!sym(paste0(x, '/', task, '/null/pred_sum'))) %>%
    dplyr::filter(motif_pair_distance <=300, motif_pair_distance >=30) %>%
    dplyr::mutate(add_coop = (hAB_sum - null_sum)/(hA_sum + hB_sum - 2*null_sum),
                  log_coop = log(add_coop)) %>%
    dplyr::group_by(motif_pair_ordered) %>%
    dplyr::summarize(median_log_add_coop = median(log_coop, na.rm = T),
                     count = dplyr::n()/2) %>%
    dplyr::mutate(motif_pair_ordered = factor(motif_pair_ordered, levels = motif_pair_blind_order)) %>%
    as.data.frame %>%
    dplyr::filter(count>=count_threshold) %>%
...
}) %>% rbindlist() %>% tidyr::pivot_wider(names_from = model, values_from  = median_log_add_coop)

#Join WT values
original_summary.df<-perturbs.df %>%
  dplyr::mutate(log_add_coop = log(add_coop)) %>%
  dplyr::filter(task=='atac', motif_pair_distance <=300, motif_pair_distance >=30) %>%
  dplyr::group_by(motif_pair_ordered) %>%
  dplyr::summarize(atac_wt_original = median(log_add_coop, na.rm = T))

all_summary.df<-alternate_model_summary.df %>% 
  dplyr::left_join(original_summary.df %>% dplyr::select(motif_pair_ordered, atac_wt_original))
readr::write_tsv(all_summary.df, 'tsv/motif_cooperativities_across_models.tsv')
```
