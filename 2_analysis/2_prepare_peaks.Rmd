...
repeats.gr<-readRDS('/n/projects/mw2098/genomes/mm10/repeatmasker.mm10.gr.rds') %>%
  plyranges::filter(grepl('LINE|ERV', repeat_class))
...
peaks.list<-list(mesc_atac_peaks = 'idr/mesc_native_atac_2_vs_3_idr.txt',
                 ...
                 bunina_atac_day0_peaks = 'idr/Bunina_mesc_atac_day0_1_vs_2_idr.txt',
                 bunina_atac_day4_peaks = 'idr/Bunina_mesc_atac_day4_1_vs_2_idr.txt',
                 bunina_atac_day8_peaks = 'idr/Bunina_mesc_atac_day8_1_vs_2_idr.txt',
                 bunina_atac_day12_peaks = 'idr/Bunina_mesc_atac_day12_1_vs_2_idr.txt'
)
```
...
peaks.gr.list<-lapply(names(peaks.list), function(x){
  ...
  gr<-df %>%
    ...
    plyranges::mutate(is_erv = overlapsAny(., repeats.gr, ignore.strand = T))
...
## Compare ATAC-seq peaks to binding peaks

```{r}
#Collect ATAC-seq peaks
atac_peaks.gr<-peaks.gr.list$mesc_atac_peaks %>%
  plyranges::mutate(signalValue_quantile = ecdf(signalValue)(signalValue),
                    atac_signal = regionSums(., 'bw/mesc_native_atac_cutsites_combined.bw'))

#Collect binding peaks
bind_peaks.gr<-grep('nexus', names(peaks.gr.list), value = T) %>% grep('bunina', ., value = T, invert = T) %>% lapply(., function(x) peaks.gr.list[[x]] %>% plyranges::mutate(name = x)) %>% as('GRangesList') %>% unlist() %>%
  plyranges::mutate(atac_signal = regionSums(., 'bw/mesc_native_atac_cutsites_combined.bw'))

#Compare quantile distributions between the two features
a_75<-quantile(atac_peaks.gr$atac_signal, .75)
a_25<-quantile(atac_peaks.gr$atac_signal, .25)

b_75<-quantile(bind_peaks.gr$atac_signal, .75)
b_25<-quantile(bind_peaks.gr$atac_signal, .25)

message('q3: ', a_75, ', q1: ', a_25, ', iqr: ', a_75-a_25)
message('q3: ', b_75, ', q1: ', b_25, ', iqr: ', b_75-b_25)

filtered_peaks_post_preparebed.gr<-rtracklayer::import('bed/bpreveal/atac_wt_fold1_all.bed')
overlapsAny(filtered_peaks_post_preparebed.gr, atac_peaks.gr, ignore.strand = T) %>% sum
length(filtered_peaks_post_preparebed.gr)
```
...
max_non_peak_reads*low_count_threshold
...
# Reviewer request: Train on expanded region set

Reviewers requested that we train (1) genome-wide, (2) on accessibility-open regions to showcase the model's robustness and to confirm we aren't missing properties of the genome. Further, I will train on a 50/50 open/not open split for a more granular transition to genome-wide, which may take some time given it is the mammalian genome. Here, we will prepare the datasets for model training.

## Genome-wide

```{r}
genome.gr<-lapply(chroms, function(x){
  GRanges(x, slidingIRanges(len = seqlengths(bsgenome)[[x]], width = output_seqlen, shift = output_seqlen))
}) %>% as('GRangesList') %>% unlist()
seqinfo(genome.gr)<-seqinfo(bsgenome)
seqlevels(genome.gr)<-seqlevels(bsgenome)

genome.gr<-genome.gr %>% 
  check_chromosome_boundaries(., 5000, bsgenome) %>%
    plyranges::select(-seqlength_filler, -end_distance, -start_distance) %>%
  plyranges::filter(!overlapsAny(GenomicRanges::resize(., floor(output_seqlen*2.5), 'center'), blacklist.gr))  %>%#2K is approximate receptive field increase
  plyranges::mutate(seq = getSeq(bsgenome, 
                                 resize(., width = floor(output_seqlen*2.5), fix = 'center'), #generally the size of the input window
                                 as.character=T)) %>%  
  plyranges::filter(!grepl('N', seq)) %>% #Filter out all regions with N's in them
  plyranges::select(-seq) #2K is approximate receptive field increase

rtracklayer::export(genome.gr, 'bed/mm10_genome_across_sliding_windows.bed')
```

## 50/50 split

Here, we will sample non-peak loci. This time, not for low read counts, but rather to a number similar to accessible regions. By this

```{r}
atac_peak_count<-length(atac_peaks.gr)
sampled_non_peaks.gr<-sample(non_peaks.gr, atac_peak_count, replace = F)
rtracklayer::export(c(atac_peaks.gr, sampled_non_peaks.gr) %>% plyranges::mutate(score = 0), 'bed/mm10_acc_and_nonacc_split.bed')
```
