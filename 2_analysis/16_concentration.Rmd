...
Measure median accessibility for these regions.

```{r}
regions.gr$atac_obs %>% log(.) %>% summary()
med_acc<-median(regions.gr$atac_obs)
```

Measure median accessibility for regions with high-affinity Sox2 motif.

```{r}
regions.gr %>% plyranges::filter(region_id %in% c(motifs.gr %>% plyranges::filter(motif=='Sox2', seq_match_quantile_bin=='smq_3') %>% .$region_id)) %>%
  .$atac_obs %>% log(.) %>% summary()
```
...
grouped.plot<-lapply(c('Oct4-Sox2', 'Sox2'), function(x){
  ...
  print('Island count of motif arrangment')
  print(grouped.df %>% dplyr::select(region_id, island_count) %>% unique %>% .$island_count %>% table)
