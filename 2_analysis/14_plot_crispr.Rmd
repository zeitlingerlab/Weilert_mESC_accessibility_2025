...
Explore the range of affinity for the low-affinity Oct4-Sox2.

```{r}
motifs.df %>% makeGRangesFromDataFrame(keep.extra.columns = T, starts.in.df.are.0based = T, seqnames.field = 'seqnames', start.field = 'start', end.field = 'end') %>%
  plyranges::filter(overlapsAny(., GRanges(coop_chrom, IRanges(coop_genomic_window[1], coop_genomic_window[2])))) %>%
  plyranges::select(seq_match, seq_match_quantile, motif, motif_id)

motifs.df %>% dplyr::arrange(seq_match_quantile) %>% dplyr::filter(motif=='Oct4-Sox2', mapping_state != 'bind') %>%
  dplyr::mutate(acc_contrib_q = ecdf(acc_contrib)(acc_contrib),
                bind_contrib_q = ecdf(bind_contrib)(bind_contrib)) %>%
  dplyr::mutate(order_by_seq = 1:nrow(.), perc_by_order = order_by_seq/nrow(.)) %>% dplyr::filter(motif_id==139897) %>%
  dplyr::select(bind_contrib_q, acc_contrib_q, order_by_seq, perc_by_order, seq_match, seq_match_quantile, motif, motif_id)
```
...
## Examine low-affinity Sox2 motif

The reviewer asks: "What happens if the low-affinity motif is deleted independently? How does accessibility change when the proximal or the low-affinity Sox2 motif are mutated in isolation, and is the sum of those less than the total accessibility measured in the native enhancer?"

So we will explicitly visualize the low-affinity motif's effects on the Akr1cl enhancer. 

```{r}
#Isolate Akr1cl motifs
akr1cl.gr<-GenomicRanges::resize(GRanges(scenarios.df$seqnames_distal, IRanges(scenarios.df$genomic_start_distal, scenarios.df$genomic_end_distal), strand = '*'), 1000, 'center')

motifs.gr<-motifs.df %>% makeGRangesFromDataFrame(keep.extra.columns = T, seqnames.field = 'seqnames', start.field = 'start', end.field = 'end')
akr1cl_motifs.gr<-subsetByOverlaps(motifs.gr, akr1cl.gr)
akr1cl_motifs.gr

#Isolate low-affinity Sox2
la_sox2.gr<-motifs.gr %>% plyranges::filter(motif_id==253083)
```


Examine motif pairs and relative cooperativity

```{r}
#Isolate low-affinity Sox2 motif pairs
pairs.df<-readr::read_tsv('tsv/genomic/all_pairs_curated_0based_w_perturb.tsv.gz') 

#Low-affinity Sox2 cooperativity
pairs.df %>% plyranges::filter(`motif_id.x`==253083) %>%
  dplyr::mutate(coop_acc = (`atac_wt/atac/hAB/pred_sum`-`atac_wt/atac/null/pred_sum`)/(`atac_wt/atac/hB/pred_sum`+`atac_wt/atac/hA/pred_sum`-(2*`atac_wt/atac/null/pred_sum`)),
                AB = `atac_wt/atac/hAB/pred_sum`-`atac_wt/atac/null/pred_sum`,
                A = `atac_wt/atac/hA/pred_sum`-`atac_wt/atac/null/pred_sum`,
                B = `atac_wt/atac/hB/pred_sum`-`atac_wt/atac/null/pred_sum`,
                coop_acc_ln = log(coop_acc),
                perturb_score = log(`atac_wt/atac/hAB/pred_sum`/`atac_wt/atac/hB/pred_sum`)) %>%
  as.data.frame() %>%
  dplyr::select(`pattern_name_unique.x`, `pattern_name_unique.y`, AB, A, B, coop_acc, `coop_acc_ln`, perturb_score, `atac_wt/atac/hAB/pred_sum`, `atac_wt/atac/hA/pred_sum`)

#Proximal Sox2 cooperativity
pairs.df %>% plyranges::filter(`motif_id.x`==253084) %>%
  dplyr::mutate(coop_acc = (`atac_wt/atac/hAB/pred_sum`-`atac_wt/atac/null/pred_sum`)/(`atac_wt/atac/hB/pred_sum`+`atac_wt/atac/hA/pred_sum`-(2*`atac_wt/atac/null/pred_sum`)),
                 AB = `atac_wt/atac/hAB/pred_sum`-`atac_wt/atac/null/pred_sum`,
                A = `atac_wt/atac/hA/pred_sum`-`atac_wt/atac/null/pred_sum`,
                B = `atac_wt/atac/hB/pred_sum`-`atac_wt/atac/null/pred_sum`,
...
ATAC-seq predicted reads from the accessibility model.

```{r}
#Plot perturbation summaries
perturbs_reads.plot<-perturbs.df %>% 
  dplyr::filter(model_name=='bpnet_osknz', genomic_position %in% c(context_genomic_window[1]:context_genomic_window[2])) %>%
  dplyr::group_by(state) %>%
  dplyr::summarize(sum_pred = sum(pred)) %>%
  ggplot(., aes(x = state, y = sum_pred))+
  geom_bar(stat = 'identity')+
...
ChIP-nexus predicted reads from the binding model.

```{r}
#Plot perturbation summaries
perturbs_reads.plot<-perturbs.df %>% 
  dplyr::filter(model_name=='bpnet_osknz_fold1', task == 'sox2', genomic_position %in% c(context_genomic_window[1]:context_genomic_window[2])) %>%
  dplyr::group_by(state) %>%
  dplyr::summarize(sum_pred = sum(pred)) %>%
  ggplot(., aes(x = state, y = sum_pred))+
  geom_bar(stat = 'identity')+
  ...
  theme_classic()
perturbs_reads.plot
ggsave(paste0(figure_filepath, '/Ark1cl_crispr_context_perturbations_summary_nexus.png'), perturbs_reads.plot, width = 7, height = 3)
ggsave(paste0(figure_filepath, '/Ark1cl_crispr_context_perturbations_summary_nexus.pdf'), perturbs_reads.plot, width = 7, height = 3)
```
...
Replot to make this less confusing as a correlation heatmap.

```{r}
cor.df<-lapply(crispr_context_nexus_reps.path %>% grep('_sox2_nexus_1_positive.bw', ., value = T), function(x){
  crispr_context_cov_w_coord.df$repa<-crispr_context_cov_w_coord.df[[x]]
  crispr_context_cov_w_coord.df$repb<-crispr_context_cov_w_coord.df[[gsub('_1_', '_2_', x)]]
  crispr_context_cov_w_coord.df$repc<-crispr_context_cov_w_coord.df[[gsub('_1_', '_3_', x)]]
  
  crispr_context_cov_w_coord.df %>% 
    dplyr::summarize(
      cor_1_v_2 = cor(repa, repb, method = 'spearman'),
      cor_1_v_3 = cor(repa, repc, method = 'spearman'),
      cor_2_v_3 = cor(repa, repc, method = 'spearman')
    ) %>%
    dplyr::mutate(bw = x)
}) %>% rbindlist() %>%
  tidyr::pivot_longer(cols = -bw, names_to = 'comparison', values_to = 'cor') %>%
  dplyr::mutate(bw = gsub('bw/mesc_Akr1cl_scenario_context_mut_', '', bw) %>% gsub('_sox2_nexus_1_positive.bw', '', .)) %>%
  dplyr::mutate(cor = round(cor, 2))

cor.plot<-ggplot(cor.df, aes(x = comparison, y = bw, fill = cor))+
  geom_tile(color = 'black')+
  geom_text(aes(label = cor))+
  scale_fill_gradient(low = 'white', high = '#b31f2c', limits = c(.7, 1))+
  theme_classic()
ggsave(paste0(figure_filepath, '/Akr1cl_crispr_context_nexus_replicate_correlations.png'), cor.plot, height = 4, width = 4)
ggsave(paste0(figure_filepath, '/Akr1cl_crispr_context_nexus_replicate_correlations.pdf'), cor.plot, height = 4, width = 4)

#Also correlate the wildtype replicates for baseline
wt.plot<-data.table(cor = cor(crispr_context_cov_w_coord.df$`bw/mesc_R1_sox2_nexus_15_positive.bw`,
                     crispr_context_cov_w_coord.df$`bw/mesc_R1_sox2_nexus_16_positive.bw`, 
                     method = 'spearman')) %>%
  ggplot(., aes(x = 1, y = 1, fill = cor))+
  geom_tile(color = 'black')+
  geom_text(aes(label = cor))+
  scale_fill_gradient(low = 'white', high = '#b31f2c', limits = c(.7, 1))+
  theme_classic()

ggsave(paste0(figure_filepath, '/Akr1cl_crispr_context_nexus_replicate_correlations.png'), cor.plot + wt.plot, height = 4, width = 8)
ggsave(paste0(figure_filepath, '/Akr1cl_crispr_context_nexus_replicate_correlations.pdf'), cor.plot + wt.plot, height = 4, width = 8)
```
