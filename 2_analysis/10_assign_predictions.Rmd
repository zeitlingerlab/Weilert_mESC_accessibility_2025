...
accessibility_genomic_motifs.path<-'tsv/genomic/motifs/accessibility_instances_formatted_for_genomic_perturbations_0based.tsv.gz'
...
alternate_models.list<-list(atac_wt_across_acc = c('atac'),
                            atac_wt_across_split = c('atac'),
                            atac_wt_across_genome = c('atac'))
```
...
# Format single and paired perturbations from accessibility models

From the accessibility model motifs (expanded set), format perturbations and paired perturbations.
...
```{r}
#Assign unique mutation ids
instances.df<-readr::read_tsv(accessibility_genomic_motifs.path) %>% 
  dplyr::group_by(region_id, motif) %>% 
  dplyr::mutate(motif_num = dplyr::row_number(),
                pattern_center = floor((motif_window_end - motif_window_start)/2) + motif_window_start - flank_length) %>%
  dplyr::ungroup() %>%
...
instances_w_perturbs.df<-lapply(instances.df$chrom %>% unique, function(x){ # %>% grep('chrX', ., invert = T, value = T)
  ...
  i_models.df<-lapply(names(models.list), function(y){
    ...
    i_task.df<-lapply(models.list[[y]], function(task){
      ...
      #For each chromosome, import perturbations
      perturbs_for_motifs.df<-readr::read_tsv(paste0('tsv/genomic/motifs/genomic_perturbs_accessibility_whole_window_', y, '_', x, '.tsv.gz'))
...
```{r}
readr::write_tsv(instances_w_perturbs.df, 'tsv/mapped_motifs/all_accessibility_instances_curated_0based_w_perturb.tsv.gz')
```
  ...
  pair_w_perturbs.df<-lapply(pair.df$`chrom.x` %>% unique, function(c){ # %>% grep('chrX', ., invert = T, value = T)
    ...
    i_models.df<-lapply(names(models.list), function(y){
      ...
      i_task.df<-lapply(models.list[[y]], function(task){
        ...
        #For each chromosome, import perturbations
        perturbs_for_motifA.df<-readr::read_tsv(paste0('tsv/genomic/motifs/genomic_perturbs_accessibility_whole_window_', y, '_', c, '.tsv.gz'))
...
readr::write_tsv(pairs_w_all_perturbs.df, 'tsv/genomic/all_accessibility_pairs_curated_0based_w_perturb.tsv.gz')
```

## Format island-level perturbations [PREP]
...
```{r}
instances_w_island_mutations.df<-lapply(instances.df$chrom %>% unique, function(x){ # %>% grep('chrX', ., invert = T, value = T)
  message(x)
  chr.df<-instances.df %>% dplyr::filter(chrom == x) %>%
  ...
  i_models.df<-lapply(c('atac_wt'), function(y){
    
    i_task.df<-lapply(models.list[[y]], function(task){
      message(x,'_', y, '_',task)
      
      #For each chromosome, import perturbations
      perturbs_for_motifs.df<-readr::read_tsv(paste0('tsv/genomic/motifs/genomic_perturbs_accessibility_island_mutations_', y, '_', x, '.tsv.gz')) %>%
        dplyr::group_by(region_id) %>%
        dplyr::mutate(island_count = max(c(stringr::str_count(mut, '_') %>% unlist())+1)) %>%
        dplyr::ungroup() %>% 
        dplyr::mutate(mut_count = c(stringr::str_count(mut, '_') %>% unlist())+1) %>%
        as.data.frame()
      
      #Get the motif name assigned to the ones that are "minus one"
      motifs_d1_index.df<-lapply(perturbs_for_motifs.df %>% dplyr::filter(island_count>1) %>% .$region_id %>% unique, function(z){
        p.df<-perturbs_for_motifs.df %>% dplyr::filter(region_id==z)
        motifs_in_region.vec<- p.df %>% dplyr::slice_max(mut_count, n = 1, with_ties = F) %>% .$mut %>% stringr::str_split(., '_') %>% unlist()
        
        p_to_label.df<-p.df %>% dplyr::filter(mut != 'Reference') %>% dplyr::slice_min(mut_count, with_ties = T)
        p_to_label.df$remaining_motif<-lapply(1:nrow(p_to_label.df), function(aa){motifs_in_region.vec[!(motifs_in_region.vec %in% c(stringr::str_split(p_to_label.df$mut[aa], '_') %>% unlist()))]}) %>% unlist()
        return(p_to_label.df)  
      }) %>% rbindlist()
      
      perturbs_for_motifs_w_rem.df<-perturbs_for_motifs.df %>% dplyr::left_join(motifs_d1_index.df)

      d0_sum<-paste0(task, '/d0/pred_sum') #mutated all motifs in island
      d1_sum<-paste0(task, '/d1/pred_sum') #mutated all motifs minus one in island
      WT_sum<-paste0(task, '/WT/pred_sum') #mutated no motifs
      dA_sum_long<-paste0(y, '/', task, '/dA/pred_sum') #mutate only motif of interest, pulled from above array
      dA_sum<-gsub(paste0(y, '/'), '', dA_sum_long)
      
      #Find the different knockout pairwise situations and collect across pairs
      i.df<- chr.df %>%
        as.data.frame() %>%
        #Get WT data
        dplyr::left_join(., perturbs_for_motifs_w_rem.df %>% 
                           dplyr::filter(mut=='Reference') %>% 
                           dplyr::rename(!!WT_sum := paste0(task, '/pred_sum')) %>%
                           dplyr::select(region_id, WT_sum), 
                         by = 'region_id') %>%
        
        #Get d0 data (when all motifs in region are mutated)
        dplyr::left_join(., perturbs_for_motifs_w_rem.df %>%
                           dplyr::group_by(region_id) %>%
                           dplyr::slice_max(mut_count, with_ties = F, n=1) %>%
                           dplyr::rename(!!d0_sum := paste0(task, '/pred_sum')) %>%
                           dplyr::select(region_id, !!d0_sum), 
                         by = c('region_id'))  %>%
      
        #Get d1 data (when all motifs in region are mutated except the motif of interest)
        dplyr::left_join(., perturbs_for_motifs_w_rem.df %>%
                           dplyr::filter(!is.na(remaining_motif)) %>%
                           dplyr::rename(!!d1_sum := paste0(task, '/pred_sum'), pattern_name_unique = remaining_motif) %>%
                           dplyr::select(region_id, pattern_name_unique, !!d1_sum), 
                         by = c('region_id', 'pattern_name_unique')) %>%
      
      #Append dA data (when only the motif of interest is mutated)
        dplyr::left_join(., instances_w_perturbs.df %>% dplyr::select(region_id, pattern_name_unique, !!dA_sum_long) %>% 
                           dplyr::rename(!!dA_sum := dA_sum_long))
        
        
      return(i.df %>% dplyr::select(!!WT_sum, !!d0_sum, !!d1_sum, !!dA_sum))
    }) %>% dplyr::bind_cols(.)
    colnames(i_task.df) <- paste0(y, '/', colnames(i_task.df))
    return(i_task.df)
  }) %>% dplyr::bind_cols(.) %>% cbind(chr.df, .)
...
```{r}
readr::write_tsv(instances_w_island_mutations.df, 'tsv/mapped_motifs/all_accessibility_instances_curated_0based_w_island_mutations.tsv.gz')
```

# Format single and paired perturbations from alternate region R1 mESC models

From the original mapped motifs, interpret using the alternate models that were trained genome-wide, acc only and split. 
...
instances_w_perturbs.df<-mclapply(instances.df$chrom %>% unique, function(x){
  ...
  i_models.df<-lapply(names(alternate_models.list), function(y){
    
    i_task.df<-lapply(alternate_models.list[[y]], function(task){
      message(x,'_', y, '_',task)
      
      #For each chromosome, import perturbations
      perturbs_for_motifs.df<-readr::read_tsv(paste0('tsv/genomic/motifs/genomic_pertubs_alternate_whole_window_', y, '_', x, '.tsv.gz'))
...
```{r}
readr::write_tsv(instances_w_perturbs.df, 'tsv/mapped_motifs/all_instances_curated_0based_w_alternate_perturb.tsv.gz')
```
  ...
  pair_w_perturbs.df<-lapply(pair.df$`chrom.x` %>% unique, function(c){
    ...
    i_models.df<-lapply(names(alternate_models.list), function(y){
      ...
      i_task.df<-lapply(alternate_models.list[[y]], function(task){
        ...
        #For each chromosome, import perturbations
        perturbs_for_motifA.df<-readr::read_tsv(paste0('tsv/genomic/motifs/genomic_pertubs_alternate_whole_window_', y, '_', c, '.tsv.gz'))
...
readr::write_tsv(pairs_w_all_perturbs.df, 'tsv/genomic/all_pairs_curated_0based_w_alternate_perturb.tsv.gz')
```
