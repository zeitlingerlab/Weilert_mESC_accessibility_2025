...
atac_obs_fragment.bw<-'bw/mesc_native_atac_combined.bw'
...
## Plot CWMs and PWMs of seqlets 

```{r}
#Plot all seqlet PWM and CWM logos
filler<-lapply(seqlet_ids.df$motif %>% unique, function(x){
  
  seqlets.df<-seqlet_ids.df %>% dplyr::filter(motif==x) %>%
    dplyr::select(pattern_name, metacluster_name, model, task, fold, oriented_strand, motif) %>%
    unique()
  
  seqlets.plot<-mclapply(1:nrow(seqlets.df), function(y){

    s.df<-seqlets.df[y,]
    s.hdf5<-rhdf5::h5read(paste0('modiscolite/', s.df$model, '_', s.df$fold, '_', s.df$task, '_counts/modisco.h5'),
                  paste0(s.df$metacluster, '/', s.df$pattern))
    cwm<-s.hdf5$contrib_scores
    pwm<-ppm_to_ic(s.hdf5$sequence, bg_freq = c(.3, .2, .2, .3))
    
    #Orient strand
    if(s.df$oriented_strand=='-'){
    ...
    y_prefix<-paste0(s.df$model_name %>% gsub('_specific', '', .) %>% gsub('histone_', '', .) %>% gsub('_strongest_gene_across_internal_cell_type_', '', .), '_', s.df$metacluster, '_', s.df$pattern)
    ...
    pwm_and_cwm.logo<-ggseqlogo(pwm_and_cwm.list, method='custom', seq_type='dna') + 
      ...
      theme(axis.text.x = element_blank())+ ggtitle(paste0(s.df$fold, '_', s.df$model))
    return(pwm_and_cwm.logo)
    
  }, mc.cores = threads) %>% wrap_plots(nrow = 1) + plot_annotation(title = x)

  ggsave(paste0(figure_filepath, '/seqlet_cwm_logo_', x, '.png'), seqlets.plot,
         width = nrow(seqlets.df)*4+1, height = 4)
  ggsave(paste0(figure_filepath, '/seqlet_cwm_logo_', x, '.pdf'), seqlets.plot,
         width = nrow(seqlets.df)*4+1, height = 4)
  return(NULL)
})
```
...
## Export accessibility-specific motifs

```{r}
#Read in accessibility hits
acc_motifs.df<-lapply(Sys.glob(paste0('modiscolite/atac_wt_fold1_*counts/hits.tsv')),
                      function(x) readr::read_tsv(x) %>%
                        dplyr::mutate(task = 'atac',
                                      model = 'atac_wt',
                                      fold = 'fold1')) %>% 
  rbindlist() %>% as.data.frame %>% 
  dplyr::left_join(., seqlet_ids.df) %>%
...
model<-'atac_wt'
acc_motifs.gr<-makeGRangesFromDataFrame(acc_motifs.df, keep.extra.columns = T, starts.in.df.are.0based = T) %>%
  plyranges::mutate(strand = ifelse(oriented_strand == '-', 
                                    ifelse(strand == '-', '+', '-'),
                                    strand))
...
#Find overlaps between motifs and peaks
ov<-findOverlaps(acc_motifs.gr, peaks.gr, ignore.strand = T)
  
#Assign temporary region ID for deduplication
acc_motifs.gr$region_id_tmp<-NA
acc_motifs.gr$region_id_tmp[ov@from]<-peaks.gr$region_id_tmp[ov@to]

#Filter any motif that doesn't fit into peaks. This means that the motif is mapped on the edges of the general, temporary region_id
acc_motifs.gr<-acc_motifs.gr %>% plyranges::filter(!is.na(acc_motifs.gr$region_id_tmp))
  
#Deduplicate any motifs for palindromic mappings
acc_dedup.gr<-lapply(unique(acc_motifs.gr$motif), function(y){
  print(y)
  gr<-acc_motifs.gr %>% plyranges::filter(motif==y)
  motifs_dedup.gr<-remove_palindromic_motifs_from_bpnet_instances(dfi = gr, 
                                                                  pattern_to_filter = y, 
                                                                  chrom_name = 'seqnames', start_name = 'start', end_name = 'end', 
                                                                  value_name = 'contrib_match', motif_name = 'motif',
                                                                  region_name = 'region_id_tmp')
  print(paste0(length(motifs_dedup.gr) , ' kept from ', length(gr)))
  return(motifs_dedup.gr)
}) %>% as('GRangesList') %>% unlist(.)

acc_final_motifs.gr<-acc_dedup.gr %>%
    plyranges::select(-c(region_id_tmp, oriented_strand, fold))

#Filter out Oct4 and Sox2 from Oct4-Sox2 motifs
#Remove Sox2 motifs overlapping with Oct4-Sox2 motif.
sox2_motifs.gr<-acc_final_motifs.gr %>% plyranges::filter(motif=='Sox2')
os_motifs.gr<-acc_final_motifs.gr %>% plyranges::filter(motif=='Oct4-Sox2')
...
dedup_w_s.gr<-c(acc_final_motifs.gr %>% plyranges::filter(motif != 'Sox2'), sox2_dedup_motifs.gr)
...
acc_final_motifs.gr<-dedup_w_o.gr
acc_final_motifs.gr$motif %>% table
```

### Calculate accessibility islands
...
```{r}
islands.gr<-resize(acc_final_motifs.gr, 150, 'center') %>%
  GenomicRanges::reduce(ignore.strand = T)
...
rtracklayer::export(islands.gr %>% 
                    ...
                    'bed/mapped_motifs/all_accessibility_islands_curated_0based.bed', format = 'BED')
rtracklayer::export(islands.gr %>% 
                      GenomicRanges::resize(., width = output_length, fix = 'center') %>% 
                      plyranges::mutate(name = region_id), 
                    'bed/mapped_motifs/all_accessibility_islands_curated_0based_sized_to_output.bed', format = 'BED')
rtracklayer::export(islands.gr %>% 
                      GenomicRanges::resize(., width = input_length, fix = 'center') %>% 
                      plyranges::mutate(name = region_id), 
                    'bed/mapped_motifs/all_accessibility_islands_curated_0based_sized_to_input.bed', format = 'BED')
```

Show how many motifs there are per window

```{r}
ov<-findOverlaps(acc_final_motifs.gr, islands.gr, ignore.strand = T)
islands_resized_to_output.gr <- islands.gr %>% GenomicRanges::resize(., width = output_length, fix = 'center')

ov@to %>% table %>% table
```

### Assign to accessibility motifs

```{r}
#Assign region_id to these motifs
acc_motifs_w_regions.gr<-acc_final_motifs.gr
acc_motifs_w_regions.gr$region_id<-NA
acc_motifs_w_regions.gr$region_id[ov@from]<-islands_resized_to_output.gr$region_id[ov@to]
acc_motifs_w_regions.gr$region_start_1based<-NA
acc_motifs_w_regions.gr$region_start_1based[ov@from]<-start(islands_resized_to_output.gr)[ov@to]
acc_motifs_w_regions.gr$region_end_1based<-NA
acc_motifs_w_regions.gr$region_end_1based[ov@from]<-end(islands_resized_to_output.gr)[ov@to]

#Filter motifs that aren't in a sufficiently small island.
acc_motifs_w_ic.gr <- acc_motifs_w_regions.gr %>% plyranges::filter(!is.na(region_id)) %>%
  plyranges::mutate(motif_id = 0:(length(.)-1), #0-based indexing later might come in handy
                    seq = getSeq(bsgenome, ., as.character = T))
```

### Export curated accessibility motifs

Show final results of motifs returned:

```{r}
acc_motifs_w_ic.gr$motif %>% table
```
...
```{r}
#1-based
acc_motifs_w_ic.df<-acc_motifs_w_ic.gr %>% as.data.frame()
readr::write_tsv(acc_motifs_w_ic.df, paste0('tsv/mapped_motifs/all_accessibility_instances_curated_1based.tsv.gz'))

#0-based
rtracklayer::export(acc_motifs_w_ic.gr %>% plyranges::mutate(name = paste0(motif, '_', motif_id, '_', region_id)), 
                    paste0('bed/mapped_motifs/all_accessibility_instances_curated_0based.bed'))
```
...
metapeak_plots.list<-mclapply(motifs_w_metadata.gr$motif %>% unique(), function(x){
  ...
  atac.df<-rbind(
    ...
    standard_metapeak(gr = resize(m.gr, 1, 'center'),
                             sample = atac_obs_fragment.bw, upstream = flank_length, downstream = flank_length) %>%
      dplyr::mutate(type = 'obs_fragment')
  )
