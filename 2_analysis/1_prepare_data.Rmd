...
featured_samples<-all_samples #%>% grep('Bunina', ., value = T) 
...
peak_cmds.df<-samples.df %>%
  dplyr::group_by(sample_group) %>%
  # dplyr::filter(sample_group %in% featured_samples) %>%
...
```{r}
# idr_cmds.vec<-lapply(samples.df %>% dplyr::filter(sample_group %in% featured_samples) %>% .$sample_group %>% unique(), function(x){
