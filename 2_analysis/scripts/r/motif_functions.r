...
seq_to_ppm<-function(seq){
  
  #Realign sequences
  seq.mat<-lapply(seq, function(x) stringr::str_split(x, '')) %>% 
    as.data.frame %>% 
    transpose() %>% 
    as.matrix()
  seq.vec<-apply(seq.mat, 2, function(x) paste0(x, collapse = ''))
  nseqs<-dim(seq.mat)[1]
  
  #Get PFM
  pfm.mat<-data.frame(A = stringr::str_count(seq.vec, 'A'),
                      C = stringr::str_count(seq.vec, 'C'),
                      G = stringr::str_count(seq.vec, 'G'),
                      T = stringr::str_count(seq.vec, 'T')) %>%
    transpose() %>%
    as.matrix()
  
  #Get PPM
  ppm.mat<-pfm.mat/nseqs
  colnames(ppm.mat)<-NULL
  
  return(ppm.mat)
}

ppm_to_pwm <- function(ppm, bg_freq){
  ppm_norm<-lapply(1:4, function(nt){
    ppm[nt,]/bg_freq[nt]
  }) %>% as.data.frame %>% as.matrix %>% t
  rownames(ppm_norm)<-c('A', 'C', 'G', 'T')
  pwm<-log2(ppm_norm)
  return(pwm)
}

ppm_to_ic <- function(ppm, bg_freq){
  pwm<-ppm_to_pwm(ppm, bg_freq)
  ic<-pmax(ppm * pwm, 0)
  ic[is.na(ic)]<-0
  return(ic)
}

get_sequence_ic<-function(seq, ic){
