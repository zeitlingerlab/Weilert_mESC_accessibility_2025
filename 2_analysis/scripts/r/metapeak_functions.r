...
format_standard_matrix_for_heatmap<-function(matrix, downstream, upstream, swap_na_with_zero = F){
...
plot_standard_heatmap<-function(regions.gr, sample, title="Heatmap of ChIP-seq Signals", order="sum", output_file_name=NA, reduce=F,
                             upstream=50, downstream=50, normalize = T,
  ...
  if(return_only_df){
    ...
    g<-ggplot()+
      ...
      scale_x_continuous("Position (bp)", expand = c(0, 0))+
      scale_y_continuous("Regions", expand = c(0, 0))+
      scale_fill_gradientn(colors = c("white", '#fff5f0', '#fee0d2','#fcbba1','#fc9272','#fb6a4a','#ef3b2c','#cb181d','#a50f15','#67000d'), name = fill_axis) +
      # scale_fill_gradient2(low="#08306b", mid="white", high="#67000d")+
      theme_classic()+
      theme(text=element_text(size=14), legend.position = "none", panel.grid = element_blank(), panel.border = element_blank())
...
plot_sequence<-function(gr, title, genome, subset=F, show=T, window=201, y_axis_name = "Regions", x_axis_name = "Motif distance (bp)", cores = 8){
  ...
  if(class(gr)=='GRanges'){
    ...
    sequences<-gr
    gr<-data.frame(row = 1:length(sequences))
  }
  ...
  if(show){print(g)}
  return(list(plot=g, df=sequence.df))
}
