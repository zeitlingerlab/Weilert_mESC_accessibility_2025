...
def compute_auprc(regions_path, profileA, profileB,
    ...
    # Assumes that the center position of the window is the middle of BPNet window as well.
    else:
        peaks_df = pd.read_csv(regions_path, sep='\t', header=None,
                               names=['chrom', 'st', 'end', 'name', 'score', 'strand'])
        peaks_df['summit'] = (peaks_df['end'] - peaks_df['st'])/2
    ...
    allPositions['start_pos'] = allPositions['start_pos'].astype(int)
    allPositions['end_pos'] = allPositions['end_pos'].astype(int)
