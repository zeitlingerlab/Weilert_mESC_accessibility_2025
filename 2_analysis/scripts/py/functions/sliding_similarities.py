...
def parallel_sliding_continousjaccard(qa, ta, pad_mode=None, n_jobs=10, verbose=True):
    ...
    r = np.stack(Parallel(n_jobs, prefer="threads")(delayed(sliding_continousjaccard)(qa, ta[i])
...
def parallel_sliding_dotproduct(qa, ta, pad_mode=None, n_jobs=10, verbose=True):
    ...
    os.environ['OMP_NUM_THREADS'] = '1'
    os.environ['MKL_DYNAMIC'] = 'FALSE'
    return pad_same(np.stack(Parallel(n_jobs, prefer="threads")(delayed(sliding_dotproduct)(qa, ta[i][np.newaxis])
...
def parallel_sliding_kl_divergence(qa, ta, kind='simmetric', pseudo_p=1e-6, pad_mode=None, n_jobs=10, verbose=True):
    ...
    return pad_same(np.concatenate(Parallel(n_jobs, prefer="threads")(delayed(sliding_kl_divergence)(qa, ta[i][np.newaxis], kind=kind)
                                                    for i in tqdm(range(len(ta)), disable=not verbose))),
                    len(qa), pad_mode)
