...
parser.add_option("-u", "--comb_max", default = None, 
                  help="Upper limit of simultaneous mutations to conduct across each region. If a str object, then denotes column in motif_groups_df to specify how many maximum motifs to consider. [default: %default]")
parser.add_option("-l", "--comb_min", default = None, 
                  help="Lower limit of simultaneous mutations to conduct across each region. If a str object, then denotes column in motif_groups_df to specify how many minimum motifs to consider. [default: %default]")
...
def bpreveal_generate_perturbations(motifs_df_path, motif_groups_df_path,
    ...
    #
    # assert (comb_min>=1) or (comb_min == None) or (type(comb_min) == str), 'Minimum combinations cannot be 0 or negative.'
    ...
    #NOTNEEDED IN THIS IMPLEMENTATION: Get motif positions relative to the input seqlen
    ...
    for chrom in tqdm(motifs_df[motif_chrom_column].unique()):
        ...
        for trial in range(trials):
            ...
            model = load_model(model_dir, custom_objects = {'multinomialNll' : losses.multinomialNll, 'reweightableMse': losses.dummyMse}, compile = False)
            preds_raw_arr = model.predict(s)
                ...
                for perturb_idx in pis_df['perturb_idx']:
                    ...
                    if use_whole_window:
                        ...
                        for idx,motif in motif_set.iterrows():
                            ...
                            # REGIONS ARE INPUT SIZED: centerpoint = int(motif[motif_window_start_column] + np.floor((motif[motif_window_end_column] - motif[motif_window_start_column])/2))
                            centerpoint = int(motif[motif_window_start_column] - flank + np.floor((motif[motif_window_end_column] - motif[motif_window_start_column])/2))
