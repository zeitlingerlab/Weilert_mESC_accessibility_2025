...
parser.add_option("--anchored_output_positionB", default = 530, type = "int",
...
parser.add_option("--affinity_output_position", default = 500, type = "int",
...
anchored_output_positionB = 530,
affinity_output_position = 500,
input_length = 2114,
output_length = 1000,
null_sequence_np_prefix = 'seqs_1he'
    ...
    for t in tqdm(range(null_seqs.shape[0])):
        ...
        for s in affinity_seqs_df.seq.values:
            ...
            null_seq = insert_motif(seq = one_hot_decode_sequence(null_seqs[t]), motif = s, position = affinity_input_position)
            seqA = insert_motif(seq = null_seq, motif = anchored_seqA, position = anchored_input_positionA)
            seqB = insert_motif(seq = null_seq, motif = anchored_seqB, position = anchored_input_positionB)
            seqAB = insert_motif(seq = seqA, motif = anchored_seqB, position = anchored_input_positionB)
            injected_seqs = injected_seqs + [null_seq, seqA, seqB, seqAB]

            #Create indexes of how the prediction arrays were assigned
            left_df = pd.DataFrame([[s]*4, [anchored_nameA]*4, [anchored_nameB]*4, ['null', 'A', 'B','AB']]).transpose()
            right_df = pd.DataFrame([[s]*4, [anchored_nameB]*4, [anchored_nameA]*4, ['null', 'B', 'A','AB']]).transpose()
