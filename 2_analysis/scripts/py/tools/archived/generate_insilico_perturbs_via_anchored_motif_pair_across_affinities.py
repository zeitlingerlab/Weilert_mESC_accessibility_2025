...
parser.add_option("--anchored_seq",
                  help="Sequence of the anchored motif to be injected")
parser.add_option("--anchored_name",
                  help="Name of the anchored motif to be injected")
parser.add_option("--anchored_output_position", default = 470, type = "int",
...
def bpreveal_generate_insilico_perturbs_via_anchored_motif_pair_across_affinities(
model_dir,
...
null_sequence_np_prefix = 'seqs_1he'
    ...
    anchored_input_position = int((input_length-output_length)/2 + anchored_output_position)
    ...
    for t in tqdm(range(null_seqs.shape[0])):
        ...
        seqA = insert_motif(seq = null_seq, motif = anchored_seq, position = anchored_input_position)

        injected_seqs = [null_seq, seqA]
        left_is_motifA_df = pd.DataFrame([['anchor_placeholder']*2, [anchored_name]*2, [affinity_name]*2, ['null','A']]).transpose()
        right_is_motifA_df = pd.DataFrame([['anchor_placeholder']*2, [affinity_name]*2, [anchored_name]*2, ['null','B']]).transpose()
        ...
        for s in affinity_seqs_df.seq.values:
            ...
            injected_seqs = injected_seqs + [seqB, seqAB]

            #Create indexes of how the prediction arrays were assigned
            left_df = pd.DataFrame([[s]*2, [anchored_name]*2, [affinity_name]*2, ['B','AB']]).transpose()
            right_df = pd.DataFrame([[s]*2, [affinity_name]*2, [anchored_name]*2, ['A','AB']]).transpose()
    ...
    preds_all_df['anchored_name'] = anchored_name
    preds_all_df['anchored_seq'] = anchored_seq
    preds_all_df.to_csv(output_tsv_path, sep = '\t', index = False)

#Run featured function
bpreveal_generate_insilico_perturbs_via_anchored_motif_pair_across_affinities(
...
anchored_seq = options.anchored_seq,
anchored_name = options.anchored_name,
affinity_txt_filepath = options.affinity_txt_filepath,
affinity_name = options.affinity_name,
anchored_output_position = options.anchored_output_position,
