...
bind_vs_acc_genomic.plot.list<-lapply(names(motif_to_task.list), function(x){
  g<-ggplot(motifs_w_centers.df %>% 
    ...
    scale_color_gradientn(colours = rev(c(inferno(5, begin = .3, end = 1))),
...
```{r}
residuals.df<-lapply(c('with_strongest', 'without_strongest'), function(distance_state){
  lapply(motifs_w_centers.df$motif %>% unique, function(x){
    lapply(c('acc', 'bind'), function(y){
      if(distance_state=='without_strongest'){
        df<-motifs_w_centers.df %>% dplyr::filter(distance_from_pioneering_center_abs<=max_distance, motif==x, distance_from_pioneering_center_abs!=0)
      } else {
        df<-motifs_w_centers.df %>% dplyr::filter(distance_from_pioneering_center_abs<=max_distance, motif==x)
      }
      
      #Find residual model
      formula.str<-as.formula(paste0(y, '_perturb ~ seq_match_quantile + distance_from_pioneering_center_abs + distance_from_peak_summit.distance + distance_from_atac_max'))
      model<-lm(formula.str, data =  df)
      af <- anova(model)
      afss <- af$"Sum Sq"
      res.df<-cbind(af,var_explained=afss/sum(afss)*100) %>%
        dplyr::mutate(motif=x, model = y, distance_state = distance_state)
      res.df$comparison<-rownames(res.df)
      res.df<-res.df %>%
...
residuals_indp.df<-lapply(c('with_strongest', 'without_strongest'), function(distance_state){
  lapply(motifs_w_centers.df$motif %>% unique, function(x){
    lapply(c('acc', 'bind'), function(y){
      if(distance_state=='without_strongest'){
        df<-motifs_w_centers.df %>% dplyr::filter(distance_from_pioneering_center_abs<=max_distance, motif==x, distance_from_pioneering_center_abs!=0)
      ...
      #Find residual model
      lapply(variables, function(readout){
        formula.str<-as.formula(paste0(y, '_perturb ~ ', readout))
      model<-lm(formula.str, data =  df)
      af <- anova(model)
      afss <- af$"Sum Sq"
      res.df<-cbind(af,var_explained=afss/sum(afss)*100) %>%
        dplyr::mutate(motif=x, model = y, distance_state = distance_state, comparison = readout)
      res.df$comparison<-rownames(res.df)
      res.df<-res.df %>%
        dplyr::filter(comparison != 'Residuals')
      return(res.df)
...
```{r}
residuals.df<-dplyr::left_join(residuals.df %>% dplyr::select(motif, var_explained, comparison, model, distance_state), 
                 residuals_indp.df %>% dplyr::select(motif, var_explained, comparison, model, distance_state) %>% dplyr::rename(var_explained_indp = var_explained)) %>%
...
plot<-residuals.df %>%
  ...
  facet_grid(distance_state ~ model)+
  theme_classic() + theme(legend.position = 'bottom')
plot
ggsave(paste0(figure_filepath, '/perturb_vs_pwm_and_dist.png'), plot, height = 5, width = 6.5)
ggsave(paste0(figure_filepath, '/perturb_vs_pwm_and_dist.pdf'), plot, height = 5, width = 6.5)

#Plot independent models
plot<-residuals.df %>%
  dplyr::filter(motif %in% c('Oct4-Sox2', 'Sox2', 'Nanog')) %>%
  ggplot(., aes(x = motif, fill = comparison, y = var_explained_indp))+
  geom_bar(stat = 'identity', position = 'dodge')+
  facet_grid(distance_state ~ model)+
  theme_classic() + theme(legend.position = 'bottom')
plot
ggsave(paste0(figure_filepath, '/perturb_vs_pwm_and_dist_independent_regression.png'), plot, height = 5, width = 6.5)
ggsave(paste0(figure_filepath, '/perturb_vs_pwm_and_dist_independent_regression.pdf'), plot, height = 5, width = 6.5)
```
...
### Depict for alternate models

Subselect perturbation scores for the alternate models.

```{r}
motifs_w_alternate_perturbs.df<-readr::read_tsv(motifs_w_alternate_perturbs.path) %>%
  dplyr::mutate(
    atac_wt_across_genome_perturb = log(`atac_wt_across_genome/atac/WT/pred_sum`) - log(`atac_wt_across_genome/atac/dA/pred_sum`),
    atac_wt_across_acc_perturb = log(`atac_wt_across_acc/atac/WT/pred_sum`) - log(`atac_wt_across_acc/atac/dA/pred_sum`),
    atac_wt_across_split_perturb = log(`atac_wt_across_split/atac/WT/pred_sum`) - log(`atac_wt_across_split/atac/dA/pred_sum`)
  )
```

```{r}
#Secondary only
df<-lapply(c('atac_wt_across_genome', 'atac_wt_across_acc', 'atac_wt_across_split'), function(x){
  message(x)
  
  motifs_w_centers.df %>% dplyr::filter(distance_from_pioneering_center_abs<=max_distance, 
                                        distance_from_pioneering_center_abs!=0) %>%
    dplyr::left_join(motifs_w_alternate_perturbs.df %>% dplyr::select(motif_id, !!sym(paste0(x, '_perturb')))) %>%
    dplyr::mutate(acc_perturb = !!sym(paste0(x, '_perturb'))) %>%
    dplyr::group_by(motif) %>%

    dplyr::summarize(acc_vs_dist = cor(acc_perturb, distance_from_pioneering_center_abs, method = 'spearman'), 
                     acc_vs_pwm = cor(acc_perturb, seq_match_quantile),
                     acc_vs_atac_max = cor(acc_perturb, distance_from_atac_max, method = 'spearman'), 
                     acc_vs_summit = cor(acc_perturb, distance_from_peak_summit.distance, method = 'spearman')) %>% as.data.frame %>%
    dplyr::filter(motif %in% c('Oct4-Sox2','Sox2', 'Nanog')) %>% as.data.frame %>% dplyr::mutate(model = x) %>%
    dplyr::mutate(motif = factor(motif, levels = c('Oct4-Sox2','Sox2', 'Nanog'))) %>% dplyr::arrange(motif)
}) %>% rbindlist() 
print(df)
readr::write_tsv(df, 'tsv/acc_vs_dist_secondary_alternate_models.tsv')
...
print(df)
readr::write_tsv(df, 'tsv/acc_vs_dist_all_alternate_models.tsv')
```
...
bind_vs_acc_genomic_vs_dist.plot<-ggplot(motifs_w_centers.df %>% 
  ...
  ggrastr::geom_point_rast(data = motifs_w_centers.df %>% 
                             ...
                             dplyr::arrange(distance_from_pioneering_center_abs), aes(x = bind_perturb, y = acc_perturb),
                           color = 'gray', size = 1.5, shape = 4)+
  ggrastr::geom_point_rast(aes(color = distance_from_pioneering_center_abs), size = 1)+
  geom_point(data = motifs_w_centers.df %>% dplyr::filter(region_id==curated_region_id, motif=='Sox2'), color = 'black')+
  ggpubr::stat_cor(method = 'spearman', label.x.npc = 'left', label.y.npc = 'top')+
  scale_color_gradientn(colours = rev(c(turbo(5, begin = 0.05, end = .95))),
                        name = 'Distance from strongest pioneer', 
                        limits = c(0, max_distance))+
  scale_x_continuous(name = 'Binding perturb. log(WT/dA)', limits = c(-.2, 1.25))+
  scale_y_continuous(name = 'Acc perturb. log(WT/dA)', limits = c(-.2, 1.25))+
  facet_grid(. ~ seq)+
  ggtitle('Sox2')+
  theme_classic() + theme(legend.position = 'bottom')
...
df<-lapply(c('atac_wt_across_genome', 'atac_wt_across_acc', 'atac_wt_across_split'), function(x){
  ...
  motifs_w_centers.df %>% dplyr::filter(distance_from_pioneering_center_abs<=max_distance, 
                                        distance_from_pioneering_center_abs!=0, motif=='Sox2', 
                                      seq %in% c(motifs_w_centers.df %>% dplyr::filter(region_id==curated_region_id, motif=='Sox2') %>% .$seq)) %>%
    dplyr::left_join(motifs_w_alternate_perturbs.df %>% dplyr::select(motif_id, !!sym(paste0(x, '_perturb')))) %>%
    dplyr::mutate(acc_perturb = !!sym(paste0(x, '_perturb'))) %>%
    dplyr::group_by(motif, seq) %>%
    ...
    dplyr::summarize(acc_vs_dist = cor(acc_perturb, distance_from_pioneering_center_abs, method = 'spearman'), 
                     acc_vs_pwm = cor(acc_perturb, seq_match_quantile),
                     acc_vs_atac_max = cor(acc_perturb, distance_from_atac_max, method = 'spearman'), 
                     acc_vs_summit = cor(acc_perturb, distance_from_peak_summit.distance, method = 'spearman'), model = x) %>% as.data.frame%>% dplyr::arrange(desc(seq))
}) %>% rbindlist() 
print(df)
readr::write_tsv(df, 'tsv/acc_vs_dist_secondary_akr1cl_alternate_models.tsv')
...
print(df)
readr::write_tsv(df, 'tsv/acc_vs_dist_all_akr1cl_alternate_models.tsv')
```
