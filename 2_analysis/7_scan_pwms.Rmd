...
#Get logodds content
logodds.list<-lapply(tf_seqlet_ids.df$motif, function(x){
  seqs<-seqlets.df.list[[x]]$seq
  seq_logodds<-ppm_to_pwm(seq_to_ppm(seqs),  bg_freq = c(.3, .2, .2, .3))
  readr::write_lines(seqs, file = paste0('memesuite/seqs/', x, '.txt'))
  return(seq_logodds)
})
names(logodds.list)<-tf_seqlet_ids.df$motif

ic.list$`Oct4-Sox2`
logodds.list$`Oct4-Sox2`
```
...
Reimplement quantile measuring for PWM scanning scores. We will be calculating the PWM match scores again (represented as logodds and information content) because FIMO does some weird rounding to speed up calculations (https://groups.google.com/g/meme-suite/c/BXv4pRCacN8). This means that our CWM-mapped scores and our FIMO-mapped scores do not operate on the exact same math (even though they're derived from the same logos).
...
all_motifs.list<-lapply(tf_seqlet_ids.df$motif, function(x){
  ...
  #Get PWM scores (information content)
  message('getting seqlets...')
  seqlet.df$ic_match_score<-mclapply(seqlet.df$seq, function(y) get_sequence_ic(ic = ic, seq = y), mc.cores = 8) %>% unlist()
  message('getting motifs...')
  motif.gr$ic_match_score<-mclapply(motif.gr$seq, function(y) get_sequence_ic(ic = ic, seq = y), mc.cores = 8) %>% unlist()
  message('getting pwms...')
  pwm.gr$ic_match_score<-mclapply(pwm.gr$seq, function(y) get_sequence_ic(ic = ic, seq = y), mc.cores = 8) %>% unlist()
  
  #Get PWM scores (logodds)
  message('getting seqlets...')
  seqlet.df$pwm_match_score<-mclapply(seqlet.df$seq, function(y) get_sequence_logodds(pwm = pwm, seq = y), mc.cores = 36) %>% unlist()
  message('getting motifs...')
  motif.gr$pwm_match_score<-mclapply(motif.gr$seq, function(y) get_sequence_logodds(pwm = pwm, seq = y), mc.cores = 36) %>% unlist()
  message('getting pwms...')
  pwm.gr$pwm_match_score<-mclapply(pwm.gr$seq, function(y) get_sequence_logodds(pwm = pwm, seq = y), mc.cores = 36) %>% unlist()

  #Get quantiles based on seqlet distributions
  ic_score_dist<-ecdf(seqlet.df$ic_match_score)
  match_score_dist<-ecdf(seqlet.df$pwm_match_score)
  
  seqlet.df$ic_match_score_q<-ic_score_dist(seqlet.df$ic_match_score)
  motif.gr$ic_match_score_q<-ic_score_dist(motif.gr$ic_match_score)
  pwm.gr$ic_match_score_q<-ic_score_dist(pwm.gr$ic_match_score)
...
Check that our calculated score is comparable to FIMO.

```{r}
cor(all_motifs.list$`Oct4-Sox2`$pwms$pwm_match_score, all_motifs.list$`Oct4-Sox2`$pwms$score)
```
...
Plot histograms of the binding effects as an alternative representation of affinity.

```{r}
sampled_motif_n <- 5000
optimal_sequence_from_contribution<-'ATTTGCATAACAATG'

bind_os.df<-all_motifs.list$`Oct4-Sox2`$motifs %>% 
  plyranges::filter(mapping_state != 'bind') %>%
  plyranges::mutate(oct4 = regionSums(GenomicRanges::resize(., 70, 'center'), 'bw/mesc_oct4_nexus_combined_grouped.bw')) %>%
  as.data.frame() %>% dplyr::rename(seq_score = pwm_match_score)
bind_os.df<-bind_os.df %>% dplyr::mutate(mm = mapply(function(x,y) sum(x!=y),strsplit(optimal_sequence_from_contribution,""),strsplit(seq,"")))

bind_histogram_log10<-bind_os.df %>%
  dplyr::mutate(bin = plyr::round_any(oct4, 10, ceiling)) %>%
  dplyr::group_by(bin) %>%
  dplyr::summarize(freq_oct4 = dplyr::n(),
                med_mm = median(mm)) %>%
  dplyr::filter(bin<=1000) %>%
  ggplot(., aes(x = bin))+
  geom_histogram(aes(y = freq_oct4, fill = med_mm), stat = 'identity')+
  scale_x_continuous(limits = c(0, 1000))+
  scale_y_log10()+
  scale_fill_gradientn(colors = inferno(4))+
  theme_classic()

bind_histogram_norm<-bind_os.df %>%
  dplyr::mutate(bin = plyr::round_any(oct4, 10, ceiling)) %>%
  dplyr::group_by(bin) %>%
  dplyr::summarize(freq_oct4 = dplyr::n(),
                med_mm = median(mm)) %>%
  ...
  scale_fill_gradientn(colors = inferno(4))+
  theme_classic()
bind_histogram<-bind_histogram_log10 + bind_histogram_norm
ggsave(paste0(figure_filepath, '/binding_histogram.png'), bind_histogram, height = 3, width = 15)
ggsave(paste0(figure_filepath, '/binding_histogram.pdf'), bind_histogram, height = 3, width = 10)
```

Re-bin things and show the groups.
...
## Show high/med/low context/isolation scores

```{r}
sampled_motif_n<-5000

filler<-mclapply(c('Klf4','Sox2','Oct4-Sox2'), function(x){
    
    #Next, do this across both binding and accessibility models
    motif_summary.df<-lapply(c('both|acc', 'both|bind', 'both|bind|acc'), function(y){
        
        message(x, y, ordering_var)
        
        #Define task-specific binding coverage
        task<-motif_to_task.list[[x]]

        #Extract all CWM scored motifs
        cwm.gr<-motifs.gr %>% plyranges::filter(motif==x, grepl(y, mapping_state)) %>%
          arrange(desc(!!sym(ordering_var))) %>%
        ...
        motif_summary.df<-c(
          cwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% head(n=sampled_motif_n) %>% dplyr::mutate(mapping = 'top_cwm'),
          cwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% head(n=floor(length(cwm.gr)/2) + floor(sampled_motif_n/2)) %>% 
            tail(n=sampled_motif_n) %>% dplyr::mutate(mapping = 'mid_cwm'),
          cwm.gr %>% arrange(!!sym(ordering_var)) %>% head(n=sampled_motif_n) %>% dplyr::mutate(mapping = 'bottom_cwm')
        ) %>%
          as.data.frame %>%
          dplyr::group_by(mapping, seq) %>% 
          #First, take median to get standard response across each individual sequence
          ...
          tidyr::separate(col = variable, into = c(NA, 'model', 'pred_type'), sep = '_') %>%
          dplyr::mutate(pred_type = factor(pred_type, c('perturb', 'marg'))) %>%
          dplyr::mutate(mapping_state = y)
    }) %>% rbindlist()
    ...
    summary.plot<-ggplot(motif_summary.df, aes(x = mapping, y = value, fill = pred_type))+
      geom_bar(stat = 'identity', position = 'dodge')+
      facet_grid(mapping_state ~ model)+
      theme_classic() + theme(legend.position = 'bottom')
    summary.plot
    ggsave(paste0(figure_filepath, '/motif_response_summary_across_affinities_', x, '_ord_by_', ordering_var, '.png'), summary.plot, height = 4, width = 4)
    ggsave(paste0(figure_filepath, '/motif_response_summary_across_affinities_', x, '_ord_by_', ordering_var, '.pdf'), summary.plot, height = 4, width = 4)

  }, mc.cores = 3)
```

## Plot PWM score distributions

```{r}
sampled_motif_n<-5000

#First, order all heatmaps either by seuqence match or total binding levels of motif
plot.list<-mclapply(names(all_motifs.list), function(x){
    ...
    lapply(c('acc', 'bind'), function(y){
      ...
      if(!((x=='Nanog') & (y=='acc'))){
        ...
        cwm.df<-cwm.gr %>% as.data.frame %>%
          dplyr::left_join(all_motifs.list[[x]]$motifs %>% as.data.frame %>% dplyr::select(motif_id, pwm_match_score)) %>%
          dplyr::arrange(desc(pwm_match_score))
        
        #Extract all PWM scored motifs
        pwm.gr<-all_motifs.list[[x]]$pwms
        pwm_only.df<-pwm.gr[overlapsAny(pwm.gr, rtracklayer::import(paste0('bed/mapped_motifs/all_', x, '_pwm_only_motifs.bed')), ignore.strand = T)] %>%
          as.data.frame %>%
          plyranges::arrange(desc(pwm_match_score)) %>% head(sampled_motif_n)
    
        #Plot PWM scores of side features
        cwm_pwm_score_side.plot<-cwm.df %>%
          dplyr::mutate(row = 1:nrow(.)) %>%
          ggplot(., aes(y = pwm_match_score, x = row))+
          ggrastr::rasterize(geom_bar(stat = 'identity', color = '#c2b59b'), dpi=300) + 
          scale_y_continuous(limits = c(0, 15), expand = c(0,0))+
          scale_x_continuous(expand=c(0,0))+
          coord_flip()+
          theme_classic()
        
        pwm_pwm_score_side.plot<-pwm_only.df %>%
          dplyr::mutate(row = 1:nrow(.)) %>%
          ggplot(., aes(y = pwm_match_score, x = row))+
          ggrastr::rasterize(geom_bar(stat = 'identity', color = '#c2b59b'), dpi=300) + 
          scale_y_continuous(limits = c(0, 15), expand = c(0,0))+
        ...
        pwm_score_side.plot<-cwm_pwm_score_side.plot + pwm_pwm_score_side.plot
        ggsave(paste0(figure_filepath, '/pwm_score_side_plot_', y, '_', x, '.png'), pwm_score_side.plot, height = 6, width = 3)
        ggsave(paste0(figure_filepath, '/pwm_score_side_plot_', y, '_', x, '.pdf'), pwm_score_side.plot, height = 6, width = 3)
      }
    })
    return(plot)
  })
...
#First, order all heatmaps either by seuqence match or total binding levels of motif
plot.list<-lapply(c('binding_levels', 'seq_match_quantile'), function(ordering_var){
  
  #Do this across all motifs
  mclapply(names(all_motifs.list), function(x){
      ...
      if(!((x=='Nanog') & (y=='acc'))){
        ...
        pwm.gr<-rtracklayer::import(paste0('bed/mapped_motifs/all_', x, '_pwm_only_motifs.bed')) %>%
          plyranges::mutate(obs_atac = regionSums(resize(., 1000, 'center'), 'bw/mesc_native_atac_combined.bw'),
                            binding_levels = regionSums(resize(., 1000, 'center'), bind.cov$pos) + abs(regionSums(resize(., 1000, 'center'), bind.cov$neg)),
                            seq_match_quantile = score)
        ...
        print(table(cwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% head(n=sampled_motif_n) %>% .$mapping_state))
        print(table(cwm.gr %>% arrange(!!sym(ordering_var)) %>% head(n=sampled_motif_n) %>% .$mapping_state))

        #Collect binding/metapeak information of top/med/bottom/PWM motifs 
        if(length(cwm.gr)<10000){
          bottom_n<-length(cwm.gr) - sampled_motif_n
        } else{
          bottom_n<-sampled_motif_n
        }
        
        bind.df<-rbind(
          pwm.gr %>% 
            arrange(desc(!!sym(ordering_var))) %>% head(n=sampled_motif_n) %>% resize(., 1, 'center') %>% 
            exo_metapeak(gr = ., sample = bind.cov, upstream = 51, downstream = 50) %>%
            dplyr::mutate(mapping = 'pwm'),
          cwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% head(n=sampled_motif_n) %>% resize(., 1, 'center') %>% 
            exo_metapeak(gr = ., sample = bind.cov, upstream = 51, downstream = 50) %>%
          ...
          cwm.gr %>% arrange(!!sym(ordering_var)) %>% head(n=bottom_n) %>% resize(., 1, 'center') %>% 
            exo_metapeak(gr = ., sample = bind.cov, upstream = 51, downstream = 50) %>%
        ...
        acc.df<-rbind(
          ...
          cwm.gr %>% arrange(desc(seq_match_quantile)) %>% head(n=floor(length(cwm.gr)/2) + floor(sampled_motif_n/2)) %>% 
            ...
            standard_metapeak(gr = ., sample = 'bw/mesc_native_atac_cutsites_combined.bw', upstream = 1001, downstream = 1000) %>%
            dplyr::mutate(mapping = 'mid_cwm'),
          cwm.gr %>% arrange(seq_match_quantile) %>% head(n=bottom_n) %>% resize(., 1, 'center')  %>%
            standard_metapeak(gr = ., sample = 'bw/mesc_native_atac_cutsites_combined.bw', upstream = 1001, downstream = 1000) %>%
            dplyr::mutate(mapping = 'bottom_cwm')
        ) %>% 
          dplyr::mutate(mapping = factor(mapping, levels = c('pwm', 'top_cwm','bottom_cwm', 'mid_cwm')))
        ...
        logo.list<-list(pwm = pwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% head(n=sampled_motif_n) %>% getSeq(bsgenome, ., as.character = T),
                        top_cwm = cwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% 
                          head(n=sampled_motif_n) %>% getSeq(bsgenome, ., as.character = T),
                        mid_cwm = cwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% 
                          head(n=floor(length(cwm.gr)/2) + floor(sampled_motif_n/2)) %>% 
                          tail(n=sampled_motif_n) %>% getSeq(bsgenome, ., as.character = T),
                        bottom_cwm = cwm.gr %>% arrange(!!sym(ordering_var)) %>% 
                          head(n=bottom_n) %>% getSeq(bsgenome, ., as.character = T))
        logo.plot<-ggplot() + geom_logo(logo.list) + theme_logo() + 
          facet_wrap(~seq_group, ncol=1, scales='free_x')  + 
          ggtitle(y)
        
        #Plot heatmps of metaplots. We will order them based on the the ordering variable assigned
        ...
        ggsave(paste0(figure_filepath, '/metaplots_', y, '_pwm_vs_cwm_across_coverage_', x, '_ord_by_', ordering_var, '.png'), plot, height = 5, width = 15)
        ggsave(paste0(figure_filepath, '/metaplots_', y, '_pwm_vs_cwm_across_coverage_', x, '_ord_by_', ordering_var, '.pdf'), plot, height = 5, width = 15)
        
        #Next, save the suballocated top/mid/bottom heatmaps of these motifs
        plot.list<-list(
          #Task based coverage
          plot_standard_heatmap_local(regions.gr = pwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% head(n=sampled_motif_n) %>% 
          ...
          plot_standard_heatmap_local(regions.gr = cwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% head(n=sampled_motif_n) %>% 
                                        plyranges::mutate(task_binding = regionSums(resize(., 100, 'center'), bind.cov$pos) + 
                                                            abs(regionSums(resize(., 100, 'center'), bind.cov$neg))) %>%
                                        plyranges::arrange(desc(task_binding)) %>%
                                        resize(., 1, 'center'),
                                      ...
                                      color_vec = c('white', '#fff7ec','#fee8c8','#fdd49e','#fdbb84','#fc8d59','#ef6548','#d7301f','#b30000','#7f0000')) + 
            theme(axis.text.y = element_blank(), axis.title.y = element_blank()),
          plot_standard_heatmap_local(regions.gr = cwm.gr %>% arrange(desc(!!sym(ordering_var))) %>% head(n=floor(length(cwm.gr)/2) + floor(sampled_motif_n/2)) %>% tail(n=sampled_motif_n) %>% 
                                        plyranges::mutate(task_binding = regionSums(resize(., 100, 'center'), bind.cov$pos) + 
                                                            abs(regionSums(resize(., 100, 'center'), bind.cov$neg))) %>%
                                        plyranges::arrange(desc(task_binding)) %>%
                                        resize(., 1, 'center'),
                                      ...
                                      color_vec = c('white', '#fff7ec','#fee8c8','#fdd49e','#fdbb84','#fc8d59','#ef6548','#d7301f','#b30000','#7f0000')) + 
            theme(axis.text.y = element_blank(), axis.title.y = element_blank()),
          plot_standard_heatmap_local(regions.gr = cwm.gr %>% arrange(!!sym(ordering_var)) %>% head(n=sampled_motif_n) %>% 
                                        plyranges::mutate(task_binding = regionSums(resize(., 100, 'center'), bind.cov$pos) + 
                                                            abs(regionSums(resize(., 100, 'center'), bind.cov$neg))) %>%
                                        plyranges::arrange(desc(task_binding)) %>%
                                        resize(., 1, 'center'),
        ...
        ggsave(paste0(figure_filepath, '/heatmaps_', y, '_pwm_vs_cwm_across_subsets_', x, '_ord_by_', ordering_var, '_alt.png'), subset_heatmaps.plot, height = 15, width = 8)
        ggsave(paste0(figure_filepath, '/heatmaps_', y, '_pwm_vs_cwm_across_subsets_', x, '_ord_by_', ordering_var, '_alt.pdf'), subset_heatmaps.plot, height = 15, width = 8)
